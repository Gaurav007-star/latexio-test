...
const FloatingToolbar = ({
  ...
  const handleGenerateLatex = async () => {
    ...
    try {
      const prompt = `
You are a LaTeX expert. Generate only the LaTeX code corresponding to this instruction, with no extra text or explanation:
"${genInput.trim()}"
      `;
...
${code}
  ...
  return (
    <div className="w-full bg-teal-300 h-max">
      <div
        className={cn(
          "middle-icons flex items-center h-max max-[1025px]:hidden",
          `w-[${size}%]`
        )}
      >
        <Dock className={cn(`w-full`)}>
              ...
              <DockIcon key={item.id}>
                <Tooltip>
                  <TooltipTrigger
                    ...
                    onClick={
                      item.id === "image" ? () => setModal(true) : item.action
                    }
                    className="flex items-center justify-center w-10 h-10 rounded cursor-pointer hover:bg-light-shadow hover:text-white transition-colors duration-150"
                  >
                    {item.iconClass && item.reactIcons ? (
                      <item.iconClass className="w-5 h-5" />
                    ) : (
                      <i className={`${item.iconClass} w-5 h-5`} />
                    )}
